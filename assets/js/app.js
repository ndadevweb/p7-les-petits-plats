...
class App {
  constructor() {
    ...
    this.selectorSearch = document.querySelector('#search')
    this.selectorTagsArea = document.querySelector('#tags-area')
    this.selectorAdvancedSearch = document.querySelector('#advanced-search')
  }
  ...
  initSearch() {
    this.selectorSearch.addEventListener('input', event => {
      this.searchValue = event.target.value.trim()
      const minSizeRequired = 3

      if(this.searchValue !== '' && this.searchValue.length < minSizeRequired) {
        return null
      }
  ...
  initAdvancedSearch() {
    ...
    this.selectorAdvancedSearch.addEventListener('click', event => {
    ...
    this.selectorAdvancedSearch.addEventListener('input', event => {
      ...
      if(advancedSearchValue === '') {
        ...
        const itemsMatches = this.advancedSearch.fetchMatches(type, advancedSearchValue)
  ...
  initTagsArea() {
    this.selectorTagsArea.addEventListener('click', event => {
      if (this.tags.isButtonRemoveClicked(event.target) === true) {
        ...
        const { recipeIngredients, recipeAppliances, recipeUstensils } = this.recipes.extractItems(recipesFiltered)
